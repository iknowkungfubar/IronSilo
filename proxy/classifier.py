...
class SemanticClassifier:
    ...
    def _score_code_request(
        ...
        code_context_count = sum(
            1 for msg in messages[:-1] if msg.get("role") == "assistant" and "```" in msg.get("content", "")
        )
    ...
    def _score_complex_reasoning(
        ...
        technical_terms = re.findall(
            r"\b(?:algorithm|architecture|framework|implementation|optimization|analysis|methodology|algorithm|strategy|approach)\b",
            text,
            re.IGNORECASE,
        )
