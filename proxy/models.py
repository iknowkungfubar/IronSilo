...
class ChatCompletionRequest(BaseModel):
    ...
    model: Optional[str] = Field(default=None, description="Model identifier (ignored by proxy, passed to upstream)")
    temperature: Optional[float] = Field(
        default=None, ge=0.0, le=2.0, description="Sampling temperature between 0 and 2"
    )
    ...
    stop: Optional[Union[str, List[str]]] = Field(default=None, description="Stop sequences for generation")
    max_tokens: Optional[int] = Field(default=None, ge=1, description="Maximum tokens to generate")
    presence_penalty: Optional[float] = Field(
        default=None, ge=-2.0, le=2.0, description="Presence penalty for repetition"
    )
    frequency_penalty: Optional[float] = Field(
        default=None, ge=-2.0, le=2.0, description="Frequency penalty for repetition"
    )
    user: Optional[str] = Field(default=None, description="Optional user identifier for tracking")
    ...
    extra_fields: Dict[str, Any] = Field(
        default_factory=dict, description="Additional fields to pass through to the upstream LLM"
    )
...
class ResponseMessage(BaseModel):
    ...
    content: Optional[str] = Field(default=None, description="The assistant's response content")
    tool_calls: Optional[List[Dict[str, Any]]] = Field(default=None, description="Tool calls made by the assistant")
...
class Choice(BaseModel):
    ...
    finish_reason: Optional[str] = Field(default=None, description="Reason the generation stopped")
...
class ChatCompletionResponse(BaseModel):
    ...
    choices: List[Choice] = Field(..., min_length=1, description="List of completion choices")
...
class ChatCompletionChunk(BaseModel):
    ...
    model: str = Field(..., description="Model used for generation")
    choices: List[StreamingChoice] = Field(..., min_length=1, description="List of streaming choices")
...
class HealthResponse(BaseModel):
    ...
    compression_enabled: bool = Field(..., description="Whether LLMLingua compression is active")
    llm_endpoint: str = Field(..., description="Configured upstream LLM endpoint")
    uptime_seconds: float = Field(..., description="Seconds since proxy started")
