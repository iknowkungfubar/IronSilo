...
class TestMiddlewareDefinitions:
    ...
    def test_auth_middleware_exists(self, traefik_config):
        ...
        assert "silk-road-auth" in middlewares or "global-auth" in middlewares, "Auth middleware must be defined"
...
class TestDockerComposeIntegration:
    ...
    def test_traefik_exposes_single_port(self, compose_config):
        ...
        assert any("8080:8080" in p for p in port_mappings), "Traefik must expose port 8080"
    ...
    def test_internal_services_have_no_exposed_ports(self, compose_config):
        ...
        for service_name in internal_services:
            ...
            if service_name == "traefik":
                continue
            assert len(ports) == 0, f"Internal service {service_name} should not expose ports directly"
    ...
    def test_internal_services_on_internal_network(self, compose_config):
        ...
        for service_name, service_config in services.items():
            ...
            assert "internal_bridge" in networks_list, f"Service {service_name} must be on internal_bridge network"
    ...
    def test_health_checks_defined(self, compose_config):
        ...
        for service_name in services_with_health:
            if service_name not in services:
                continue
            service = services[service_name]
            assert "healthcheck" in service, f"Service {service_name} must have a health check"
