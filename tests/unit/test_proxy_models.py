...
class TestChatCompletionRequest:
    ...
    def test_minimal_request(self):
        ...
        req = ChatCompletionRequest(messages=[Message(role=Role.USER, content="Hello")])
    ...
    def test_temperature_bounds(self):
        ...
        # Valid
        ChatCompletionRequest(messages=[Message(role=Role.USER, content="test")], temperature=0.0)
        ChatCompletionRequest(messages=[Message(role=Role.USER, content="test")], temperature=2.0)

        # Invalid
        with pytest.raises(ValidationError):
            ChatCompletionRequest(messages=[Message(role=Role.USER, content="test")], temperature=-0.1)
        with pytest.raises(ValidationError):
            ChatCompletionRequest(messages=[Message(role=Role.USER, content="test")], temperature=2.1)

    def test_max_tokens_bounds(self):
        """Test max_tokens validation."""
        # Valid
        ChatCompletionRequest(messages=[Message(role=Role.USER, content="test")], max_tokens=1)

        # Invalid
        with pytest.raises(ValidationError):
            ChatCompletionRequest(messages=[Message(role=Role.USER, content="test")], max_tokens=0)
...
class TestErrorResponse:
    ...
    def test_error_response_creation(self):
        ...
        err = ErrorResponse.create(message="Invalid request", type="invalid_request_error", code="missing_field")
