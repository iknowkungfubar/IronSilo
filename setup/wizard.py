...
class SetupWizard:
    ...
    def _step_select_host(self) -> None:
        ...
        while True:
            endpoint = get_input("LLM endpoint URL", "http://localhost:8000/v1/chat/completions")
    ...
    def _step_configure_features(self) -> None:
        ...
        # IronClaw agent
        self.config.enable_ironclaw = get_yes_no("Enable IronClaw autonomous agent?", default=True)
        print_success(f"IronClaw: {'enabled' if self.config.enable_ironclaw else 'disabled'}")

        # SearxNG private search
        self.config.enable_searxng = get_yes_no("Enable SearxNG private web search?", default=True)
    ...
    def _step_configure_resources(self) -> None:
        ...
        # Memory limit
        memory_input = get_input("Memory limit for containers (MB)", str(self.config.memory_limit_mb))
        try:
        ...
        # CPU limit
        cpu_input = get_input("CPU limit (cores)", str(self.config.cpu_limit))
        try:
    ...
    def _step_configure_security(self) -> None:
        ...
        self.config.postgres_user = get_input("Database user", self.config.postgres_user)

        self.config.postgres_password = get_input("Database password (min 8 chars)", self.config.postgres_password)
